...
class Shelf(InteractiveObject):
    ...
    def collision(self, x_position, y_position):
        return obj_collision(self, x_position, y_position, x_margin=0.75)
    ...
    def render(self, screen, camera):
        ...
        x_position = self.position[0] * config.SCALE - (camera.position[0] * config.SCALE)
        y_position = self.position[1] * config.SCALE - (camera.position[1] * config.SCALE)
